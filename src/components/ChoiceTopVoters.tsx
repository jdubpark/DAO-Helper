...
import { numToCompactString, truncateIfAddressOrLong } from '@/utils'
...
export default function ChoiceTopVoters(props: ChoiceTopVotersProps) {
  ...
  useEffect(() => {
    const topVotes = props.votes
      .filter((vote) => {
        ...
        if (Array.isArray(vote.choice)) return vote.choice.includes(props.choiceIndex + 1)
